...
$unit_bold1=array(18,19,23,24,26,27,28,29,137,139,140,157,158,160,162,163,192,198,199,213,226,228,256,257,274,277,278,279,282,289,291,293,299,300,301,303,304,324,356,358,359,360,364,373,375,386,387,410,413,414,420,423,474,475,476,491,493,513,515,521,523,538,579,581,598,601,602,603,605,606,607,609,673,758,762,772,776,777,780,785,788,793,796);
$unit_bold2=array_merge(range(44,114),range(127,134),range(145,152),range(166,184),range(344,352),range(425,432),range(440,469),range(485,488),range(500,509),range(526,529),range(533,536),range(544,552),range(569,576),range(587,596),range(621,661),range(678,733),range(748,755),range(767,770));
...
$unit_aqua=array(61,124,135,156,161,164,165,226,227,228,245,287,288,292,302,305,309,313,317,321,327,328,339,342,343,361,363,367,368,391,411,412,421,424,430,438,439,484,489,492,516,519,520,522,524,525,530,531,537,539,540,541,556,557,558,577,604,608,610,619,620,635,662,668,670,672,674,675,677,689,690,697,715,716,724,729,730,756,757,759,760,761,763,764,773,781,782,783,784,790,791,792);
...
$unit_tab[2]=array(219,158,133,72,139,140,141,78,142,143,197,198,199,200,201,134,40,41,42,43,238,239,240,241,242,243,244,245,246,247,248,249,250,251,252,253,263,264,265,266,267,268,269,270,271,272,273,274,275,276,277,278);//Люди
...
$unit_tab[5]=array(21,205,99,37,191,38,94,95,56,257,97,285,65,98,160,254);//Демоны
$unit_tab[6]=array(15,126,105,106,107,127,108,283,284);//Эльфы
...
/*
$UP_MASK[$unit][$button][$up] = array - панель изменения параметров юнитов при апгрейдах (с помощью абилок и кнопок)
(0=Жизнь,1=Атака,2=Контратака,3=Защита,4=Защита от выстрела,5=Сопротивление,
6=Скорость,7=Дистанционная атака,8=Дальность выстрела,9=Запас снарядов,10=Выносливость,11=Боевой дух)
*/

function up_abil_add($unit, $idx, $up)//возможные изменения параметров юнитов при апгрейдах (с помощью абилок и кнопок)
{
	global $UP_MASK, $only_once;
	switch($up)
	{
		case   1: $UP_MASK[$unit][$idx]{0} += 1;	break;
		case   2: $UP_MASK[$unit][$idx]{0} += 2;	break;
		case   3: $UP_MASK[$unit][$idx]{0} += 3;	break;
		case   4: $UP_MASK[$unit][$idx]{1} += 1;	break;
		case   5: $UP_MASK[$unit][$idx]{1} += 2;	break;
		case   6: $UP_MASK[$unit][$idx]{1} += 3;	break;
		case   7: $UP_MASK[$unit][$idx]{2} += 1;	break;
		case   8: $UP_MASK[$unit][$idx]{2} += 2;	break;
		case   9: $UP_MASK[$unit][$idx]{2} += 3;	break;
		case  10: $UP_MASK[$unit][$idx]{3} += 1;	break;
		case  11: $UP_MASK[$unit][$idx]{3} += 2;	break;
		case  12: $UP_MASK[$unit][$idx]{3} += 3;	break;
		case  13: $UP_MASK[$unit][$idx]{4} += 1;	break;
		case  14: $UP_MASK[$unit][$idx]{4} += 2;	break;
		case  15: $UP_MASK[$unit][$idx]{4} += 3;	break;
		case  16: $UP_MASK[$unit][$idx]{5} += 1;	break;
		case  17: $UP_MASK[$unit][$idx]{5} += 2;	break;
		case  19: $UP_MASK[$unit][$idx]{6} += 1;	break;
		case  20: $UP_MASK[$unit][$idx]{7} += 1;	break;
		case  21: $UP_MASK[$unit][$idx]{7} += 2;	break;
		case  22: $UP_MASK[$unit][$idx]{7} += 3;	break;
		case  23: $UP_MASK[$unit][$idx]{8} += 1;	break;
		case  24: $UP_MASK[$unit][$idx]{8} += 2;	break;
		case  25: $UP_MASK[$unit][$idx]{0} += 4;	break;
		case  27: $UP_MASK[$unit][$idx]{9} += 1;	break;
		case  28: $UP_MASK[$unit][$idx]{9} += 2;	break;
		case  29: $UP_MASK[$unit][$idx]{9} += 3;	break;
		case  30: $UP_MASK[$unit][$idx]{10} += 1;	break;
		case  31: $UP_MASK[$unit][$idx]{10} += 2;	break;
		case  32: $UP_MASK[$unit][$idx]{10} += 3;	break;
		case  33: $UP_MASK[$unit][$idx]{11} += 1;	break;
		case  34: $UP_MASK[$unit][$idx]{11} += 2;	break;
		case  35: $UP_MASK[$unit][$idx]{11} += 3;	break;
		case  36: $UP_MASK[$unit][$idx]{10} += 1;	$UP_MASK[$unit][$idx]{0} += 1;	break;
		case  37: $UP_MASK[$unit][$idx]{10} += 2;	$UP_MASK[$unit][$idx]{0} += 2;	break;
		case  38: $UP_MASK[$unit][$idx]{1} += 1;	$UP_MASK[$unit][$idx]{2} += 1;	break;
		case  39: $UP_MASK[$unit][$idx]{1} += 2;	$UP_MASK[$unit][$idx]{2} += 2;	break;
		case  40: $UP_MASK[$unit][$idx]{3} += 1;	$UP_MASK[$unit][$idx]{4} += 1;	break;
		case  41: $UP_MASK[$unit][$idx]{3} += 2;	$UP_MASK[$unit][$idx]{4} += 2;	break;
		case  42: $UP_MASK[$unit][$idx]{5} += 1;	$UP_MASK[$unit][$idx]{11} += 1;	break;
		case  43: $UP_MASK[$unit][$idx]{5} += 2;	$UP_MASK[$unit][$idx]{11} += 2;	break;
		case 293: $UP_MASK[$unit][$idx]{7} += 1;	break;
		case 447: if(!$once_flag[$idx][$up]) {$UP_MASK[$unit][$idx]{6} += 1; $once_flag[$idx][$up] = TRUE;}	break;
		case 452: if(!$once_flag[$idx][$up]) {$UP_MASK[$unit][$idx]{6} += 1; $once_flag[$idx][$up] = TRUE;}	break;
		case 501: $UP_MASK[$unit][$idx]{7} += 2;	break;
		case 606: $UP_MASK[$unit][$idx]{8} += 1;	break;
		case 672: if(!$once_flag[$idx][$up]) {$UP_MASK[$unit][$idx]{7} += 2; $once_flag[$idx][$up] = TRUE;}	break;
		case 748: $UP_MASK[$unit][$idx]{7} += 1;	$UP_MASK[$unit][$idx]{1} += 1;	break;
		case 761: $UP_MASK[$unit][$idx]{7} += 1;	$UP_MASK[$unit][$idx]{8} += 1;	break;
		case 763: if(!$once_flag[$idx][$up]) {$UP_MASK[$unit][$idx]{7} += 10; $UP_MASK[$unit][$idx]{8} += 5; $once_flag[$idx][$up] = TRUE;}	break;
		case 765: $UP_MASK[$unit][$idx]{7} += 1;	break;
		case 766: $UP_MASK[$unit][$idx]{7} += 2;	break;
		case 770: $UP_MASK[$unit][$idx]{8} += 1;	break;
		case 773: if(!$once_flag[$idx][$up]) {$UP_MASK[$unit][$idx]{7} += 8; $UP_MASK[$unit][$idx]{8} += 1; $once_flag[$idx][$up] = TRUE;}	break;
		case 774: $UP_MASK[$unit][$idx]{7} += 1;	break;
		case 775: $UP_MASK[$unit][$idx]{7} += 2;	break;
		case 779: $UP_MASK[$unit][$idx]{7} += 2;	break;
		case 780: $UP_MASK[$unit][$idx]{8} += 1;	break;
		case 781: if(!$once_flag[$idx][$up]) {$UP_MASK[$unit][$idx]{0} += 3; $once_flag[$idx][$up] = TRUE;}	break;
		case 782: if(!$once_flag[$idx][$up]) {$UP_MASK[$unit][$idx]{3} += 2; $UP_MASK[$unit][$idx]{4} += 2; $once_flag[$idx][$up] = TRUE;}	break;
		case 783: if(!$once_flag[$idx][$up]) {$UP_MASK[$unit][$idx]{7} += 1; $UP_MASK[$unit][$idx]{9} += 1; $once_flag[$idx][$up] = TRUE;}	break;
	}
}
...
dumper($unit_tab_array,"unit_tab_array");
...
for($i = 0,$n=0; $i < $count_u; $i++)
{
	unset($once_flag);//чтобы не считать много раз абилки с Only Once: 1 (для макс значений параметров юнита)
	if(eregi("^/([0-9]{1,})",$u_file[$i],$k))
    {
		$n=$k[1];
//		if($n>$max_u)$max_u=$n;
...
//echo "SUB=".substr(trim($s2[0]),0,1)." s2=".$s2[0]." ".$s2[1]."<br>";
				$uu=$s2[0]+1-1; // 3
				$hrom[$n][1][$j] .= $up_name[$uu].",[".($s2[1]+1-1)."]; ";//описание юнитов от Хрома: Способность, [шанс получения] - Жизнь +1, [1]
...
//					if(!array_search($up_need[$uu],$unit_path_abil[$n]))
					if(in_array($up_need[$uu],$unit_path_abil[$n]))//апы, ассоциированные с кнопками выбора пути апгрейда
					{
						...
						if($idx == 0)//левая кнопка
						{
							...
							$up_add[0] = $uu;//возможные изменения параметров юнитов при апгрейдах (с помощью абилок) - отдельно для каждого пути развития
						}
						else//правая кнопка
						{
							if($u_abil_c_path_1[$n][$uu]=="")$u_abil3_path_1[$n][$j][]=$uu;
							$u_abil_c_path_1[$n][$uu]++;
							$up_add[1] = $uu;//возможные изменения параметров юнитов при апгрейдах (с помощью абилок) - отдельно для каждого пути развития
						}
					}
					else
					{
						...
						$u_abil_c[$n][$uu]++;
						$up_add[2] = $uu;//возможные изменения параметров юнитов при апгрейдах (с помощью абилок) - общие абилки
					}
...
//возможные изменения параметров юнитов при апгрейдах (с помощью абилок)
					...
					{
						foreach($up_add as $idx => $uu_add)
						{
							up_abil_add($n, $idx, $uu_add);
/*							if($uu_add==1) eval('$life_table'.$idx.'[$n] += 1;');
							if($uu_add==2) eval('$life_table'.$idx.'[$n] += 2;');
							if($uu_add==3) eval('$life_table'.$idx.'[$n] += 3;');
							if($uu_add==4) eval('$attack_table'.$idx.'[$n] += 1;');
							if($uu_add==5) eval('$attack_table'.$idx.'[$n] += 2;');
							if($uu_add==6) eval('$attack_table'.$idx.'[$n] += 3;');
							if($uu_add==7) eval('$c_attack_table'.$idx.'[$n] += 1;');
							if($uu_add==8) eval('$c_attack_table'.$idx.'[$n] += 2;');
							if($uu_add==9) eval('$c_attack_table'.$idx.'[$n] += 3;');
							if($uu_add==10) eval('$defence_table'.$idx.'[$n] += 1;');
							if($uu_add==11) eval('$defence_table'.$idx.'[$n] += 2;');
							if($uu_add==12) eval('$defence_table'.$idx.'[$n] += 3;');
							if($uu_add==13) eval('$r_defence_table'.$idx.'[$n] += 1;');
							if($uu_add==14) eval('$r_defence_table'.$idx.'[$n] += 2;');
							if($uu_add==15) eval('$r_defence_table'.$idx.'[$n] += 3;');
							if($uu_add==16) eval('$resist_table'.$idx.'[$n] += 1;');
							if($uu_add==17) eval('$resist_table'.$idx.'[$n] += 2;');
							if($uu_add==19) eval('$speed_table'.$idx.'[$n] += 1;');
							if($uu_add==20) eval('$r_attack_table'.$idx.'[$n] += 1;');
							if($uu_add==21) eval('$r_attack_table'.$idx.'[$n] += 2;');
							if($uu_add==22) eval('$r_attack_table'.$idx.'[$n] += 3;');
							if($uu_add==23) eval('$s_range_table'.$idx.'[$n] += 1;');
							if($uu_add==24) eval('$s_range_table'.$idx.'[$n] += 2;');
							if($uu_add==25) eval('$life_table'.$idx.'[$n] += 4;');
							if($uu_add==27) eval('$ammo_table'.$idx.'[$n] += 1;');
							if($uu_add==28) eval('$ammo_table'.$idx.'[$n] += 2;');
							if($uu_add==29) eval('$ammo_table'.$idx.'[$n] += 3;');
							if($uu_add==30) eval('$stamina_table'.$idx.'[$n] += 1;');
							if($uu_add==31) eval('$stamina_table'.$idx.'[$n] += 2;');
							if($uu_add==32) eval('$stamina_table'.$idx.'[$n] += 3;');
							if($uu_add==33) eval('$morale_table'.$idx.'[$n] += 1;');
							if($uu_add==34) eval('$morale_table'.$idx.'[$n] += 2;');
							if($uu_add==35) eval('$morale_table'.$idx.'[$n] += 3;');
							if($uu_add==36) {eval('$stamina_table'.$idx.'[$n] += 1;'); eval('$life_table'.$idx.'[$n] += 1;');}
							if($uu_add==37) {eval('$stamina_table'.$idx.'[$n] += 2;'); eval('$life_table'.$idx.'[$n] += 2;');}
							if($uu_add==38) {eval('$attack_table'.$idx.'[$n] += 1;'); eval('$c_attack_table'.$idx.'[$n] += 1;');}
							if($uu_add==39) {eval('$attack_table'.$idx.'[$n] += 2;'); eval('$c_attack_table'.$idx.'[$n] += 2;');}
							if($uu_add==40) {eval('$defence_table'.$idx.'[$n] += 1;'); eval('$r_defence_table'.$idx.'[$n] += 1;');}
							if($uu_add==41) {eval('$defence_table'.$idx.'[$n] += 2;'); eval('$r_defence_table'.$idx.'[$n] += 2;');}
							if($uu_add==42) {eval('$resist_table'.$idx.'[$n] += 1;'); eval('$morale_table'.$idx.'[$n] += 1;');}
							if($uu_add==43) {eval('$resist_table'.$idx.'[$n] += 2;'); eval('$morale_table'.$idx.'[$n] += 2;');}
							if($uu_add==293) eval('$r_attack_table'.$idx.'[$n] += 1;');
							eval('if(($uu_add==447) && !$once_flag['.$idx.'][$uu_add]) {$speed_table'.$idx.'[$n] += 1; $once_flag['.$idx.'][$uu_add] = TRUE;}');
//							if($uu_add==447) eval('$speed_table'.$idx.'[$n] += 1;');//Only Once: 1
							eval('if(($uu_add==452) && !$once_flag['.$idx.'][$uu_add]) {$speed_table'.$idx.'[$n] += 1; $once_flag['.$idx.'][$uu_add] = TRUE;}');
//							if($uu_add==452) eval('$speed_table'.$idx.'[$n] += 1;');//Only Once: 1
							if($uu_add==501) eval('$r_attack_table'.$idx.'[$n] += 2;');
							if($uu_add==606) eval('$s_range_table'.$idx.'[$n] += 1;');
							eval('if(($uu_add==672) && !$once_flag['.$idx.'][$uu_add]) {$r_attack_table'.$idx.'[$n] += 2; $once_flag['.$idx.'][$uu_add] = TRUE;}');
//							if($uu_add==672) eval('$r_attack_table'.$idx.'[$n] += 2;');//Only Once: 1
//							if($uu_add==715) {$r_attack_table2[$n] += 3; $s_range_table2[$n] += 1;}//Name: Осадный режим (Снайпер);
//							if($uu_add==716) {$r_attack_table2[$n] += 8; $s_range_table2[$n] += 2;}//Name: Осадный режим (Гномья пушка);
							if($uu_add==748) {eval('$r_attack_table'.$idx.'[$n] += 1;'); eval('$attack_table'.$idx.'[$n] += 1;');}
							if($uu_add==761) {eval('$r_attack_table'.$idx.'[$n] += 1;'); eval('$s_range_table'.$idx.'[$n] += 1;');}
							eval('if(($uu_add==763) && !$once_flag['.$idx.'][$uu_add]) {$r_attack_table'.$idx.'[$n] += 10; $s_range_table'.$idx.'[$n] += 5; $once_flag['.$idx.'][$uu_add] = TRUE;}');
//							if($uu_add==763) {eval('$r_attack_table'.$idx.'[$n] += 10;'); eval('$s_range_table'.$idx.'[$n] += 5;');}//Only Once: 1
//							eval('if(($uu_add==765) && !$once_flag['.$idx.'][$uu_add]) {$r_attack_table'.$idx.'[$n] += 1; $once_flag['.$idx.'][$uu_add] = TRUE;}');
							if($uu_add==765) eval('$r_attack_table'.$idx.'[$n] += 1;');//Only Once: 1
//							eval('if(($uu_add==766) && !$once_flag['.$idx.'][$uu_add]) {$r_attack_table'.$idx.'[$n] += 2; $once_flag['.$idx.'][$uu_add] = TRUE;}');
							if($uu_add==766) eval('$r_attack_table'.$idx.'[$n] += 2;');//Only Once: 1
							if($uu_add==770) eval('$s_range_table'.$idx.'[$n] += 1;');
							eval('if(($uu_add==773) && !$once_flag['.$idx.'][$uu_add]) {$r_attack_table'.$idx.'[$n] += 8; $s_range_table'.$idx.'[$n] += 1; $once_flag['.$idx.'][$uu_add] = TRUE;}');
//							if($uu_add==773) {eval('$r_attack_table'.$idx.'[$n] += 8;'); eval('$s_range_table'.$idx.'[$n] += 1;');}//Only Once: 1
//							eval('if(($uu_add==774) && !$once_flag['.$idx.'][$uu_add]) {$r_attack_table'.$idx.'[$n] += 1; $once_flag['.$idx.'][$uu_add] = TRUE;}');
							if($uu_add==774) eval('$r_attack_table'.$idx.'[$n] += 1;');//Only Once: 1
//							eval('if(($uu_add==775) && !$once_flag['.$idx.'][$uu_add]) {$r_attack_table'.$idx.'[$n] += 2; $once_flag['.$idx.'][$uu_add] = TRUE;}');
							if($uu_add==775) eval('$r_attack_table'.$idx.'[$n] += 2;');//Only Once: 1
//							eval('if(($uu_add==779) && !$once_flag['.$idx.'][$uu_add]) {$r_attack_table'.$idx.'[$n] += 2; $once_flag['.$idx.'][$uu_add] = TRUE;}');
							if($uu_add==779) eval('$r_attack_table'.$idx.'[$n] += 2;');//Only Once: 1
							if($uu_add==780) eval('$s_range_table'.$idx.'[$n] += 1;');
							eval('if(($uu_add==781) && !$once_flag['.$idx.'][$uu_add]) {$life_table'.$idx.'[$n] += 3; $once_flag['.$idx.'][$uu_add] = TRUE;}');
//							if($uu_add==781) eval('$life_table'.$idx.'[$n] += 3;');//Only Once: 1
							eval('if(($uu_add==782) && !$once_flag['.$idx.'][$uu_add]) {$defence_table'.$idx.'[$n] += 2; $r_defence_table'.$idx.'[$n] += 2; $once_flag['.$idx.'][$uu_add] = TRUE;}');
//							if($uu_add==782) {eval('$defence_table'.$idx.'[$n] += 2;'); eval('$r_defence_table'.$idx.'[$n] += 2;');}//Only Once: 1
							eval('if(($uu_add==783) && !$once_flag['.$idx.'][$uu_add]) {$r_attack_table'.$idx.'[$n] += 1; $ammo_table'.$idx.'[$n] += 1; $once_flag['.$idx.'][$uu_add] = TRUE;}');
//							if($uu_add==783) {eval('$r_attack_table'.$idx.'[$n] += 1;'); eval('$ammo_table'.$idx.'[$n] += 1;');}//Only Once: 1
*/
						}
//						$up_add[0] = $up_add[1] = $up_add[2] = "";
						unset($up_add);
					}
				}//Положительный вес
			}
...
//					if((!in_array($s2_0,$u_loot_lvl[$n])) && ($s2_0 != 0))	// 3
	...
	//					$u_loot_lvl[$n][]=$s2_0; //массив уже даденных артов
						...
						$hrom[$n][2][$j] .= $item_name[$s2_0].",[".$s2_1."]; ";//описание юнитов от Хрома: Предмет, [шанс выпадения в трофеях] - Набор лекаря, [1]
					}
					if(isset($item_summon[$s2_0]) && $item_rarity[$s2_0] <= 6)//"Способы получения" через свиток в луте
					{
						if($summon_flag == 0)
						{
...
//echo $n." ".$j." loot=".$u_loot_lvl[$n][$s2[0]+1-1]."<br>";
			...
			if(isset($hrom[$n][1][$j]))
				$hrom[$n][1][$j] = substr($hrom[$n][1][$j],0,-2);
			if(isset($hrom[$n][2][$j]))
				$hrom[$n][2][$j] = substr($hrom[$n][2][$j],0,-2);
			$i++;
		}
		$u_loot_prn[$n] = substr($u_loot_prn[$n],0,-2);
//		$hrom[$n][2] = substr($hrom[$n][2],0,-2);
	}
...
//dumper($r_attack_table0,"r_attack_table0");
//dumper($UP_MASK,"UP_MASK");
...
//echo $n."-".$s1."<br>";	
	...
	if(eregi("^Upgraded:",$a_file[$i]))
	{
		...
		$up1=$s2[0]+1-1;
		$up2=$s2[1]+1-1;
		if($up1!=0) $upgraded1[$n] = $up1;
		if($up2!=0) $upgraded2[$n] = $up2;
		if($up1 > 0)//Повышение
		{
...
//формирование описания юнитов от Хрома
$f=fopen("unit2.txt","w") or die("Ошибка при создании файла unit2.txt");
fwrite($f,"0. Пусто\r\n");
fwrite($f,"#Нет описания#\r\n\r\n");
for($i = 1; $i <= $max_u; $i++)
{
	fwrite($f,$i.". ".$u_name[$i]."\r\n#");
	if(isset($upgraded1[$i]))
	{
		fwrite($f,"Повышение: ".$u_name[$upgraded1[$i]]."[Ур.".$upgraded2[$i]."]\r\n\r\n");
	}
	fwrite($f,"(Ур.): Апгрейд,[шанс получения]\r\n--------------------------------\r\n");
	foreach($hrom[$i][1] as $lvl => $up)
	{
		fwrite($f,"(".$lvl."): ".$up."\r\n");
	}
	if(isset($hrom[$i][2]))
	{
		fwrite($f,"\r\n(Ур.): Предмет,[шанс в трофеях]\r\n--------------------------------\r\n");
		foreach($hrom[$i][2] as $lvl => $up)
		{
			fwrite($f,"(".$lvl."): ".$up."\r\n");
		}
	}
...
//формирование параметров юнитов: начальных и максимальных
for($i=1;$i<$max_u+1;$i++)
{
/*
$UP_MASK[$unit][$button] = array - панель изменения параметров юнитов при апгрейдах (с помощью абилок и кнопок)
(0=Жизнь,1=Атака,2=Контратака,3=Защита,4=Защита от выстрела,5=Сопротивление,
6=Скорость,7=Дистанционная атака,8=Дальность выстрела,9=Запас снарядов,10=Выносливость,11=Боевой дух)
*/

/*	foreach($UP_MASK[$i] as $idx => $up)
	{
		$up_prn[$i][$up] = "<br>&#150;<br>";
		if($idx != 2)//пути развития
			$up_prn[$i][$up] .= "<font color=\"#FFCC00\">";
		$sum[$idx] = max(
	}
*/
	for($j=0;$j<13;$j++)
	{
		$up_prn[$i][$j][1] = $UP_MASK[$i][2][$j] + max($UP_MASK[$i][0][$j], $UP_MASK[$i][1][$j]);//дельта для прибавки к начальному значению апа
		if($up_prn[$i][$j][1] != 0)
		{
			$up_prn[$i][$j][0] = "<br>&#150;<br>";//префикс макс апа
			if(isset($UP_MASK[$i][0][$j]) || isset($UP_MASK[$i][1][$j]))
			{
				$up_prn[$i][$j][0] .= "<font color=\"#FFCC00\">";
				$up_prn[$i][$j][2] = "</font>";//постфикс макс апа
			}
		}
	}
	$life_table_prn[$i] = "".$life_table[$i];
	if(isset($life_table0[$i]) || isset($life_table1[$i]) || isset($life_table2[$i]))
	{
		$life_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($life_table0[$i]) || isset($life_table1[$i]))
			$life_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($life_table0[$i] > $life_table1[$i])
		{
			$life_table_prn[$i] .= ($life_table[$i] + $life_table0[$i] + $life_table2[$i]);
		}
		else
		{
			$life_table_prn[$i] .= ($life_table[$i] + $life_table1[$i] + $life_table2[$i]);
		}
		if(isset($life_table0[$i]) || isset($life_table1[$i]))
			$life_table_prn[$i] .= "</font>";
	}

	$attack_table_prn[$i] = "".$attack_table[$i];
	if(isset($attack_table0[$i]) || isset($attack_table1[$i]) || isset($attack_table2[$i]))
	{
		$attack_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($attack_table0[$i]) || isset($attack_table1[$i]))
			$attack_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($attack_table0[$i] > $attack_table1[$i])
		{
			$attack_table_prn[$i] .= ($attack_table[$i] + $attack_table0[$i] + $attack_table2[$i]);
		}
		else
		{
			$attack_table_prn[$i] .= ($attack_table[$i] + $attack_table1[$i] + $attack_table2[$i]);
		}
		if(isset($attack_table0[$i]) || isset($attack_table1[$i]))
			$attack_table_prn[$i] .= "</font>";
	}

	$c_attack_table_prn[$i] = "".$c_attack_table[$i];
	if(isset($c_attack_table0[$i]) || isset($c_attack_table1[$i]) || isset($c_attack_table2[$i]))
	{
		$c_attack_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($c_attack_table0[$i]) || isset($c_attack_table1[$i]))
			$c_attack_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($c_attack_table0[$i] > $c_attack_table1[$i])
		{
			$c_attack_table_prn[$i] .= ($c_attack_table[$i] + $c_attack_table0[$i] + $c_attack_table2[$i]);
		}
		else
		{
			$c_attack_table_prn[$i] .= ($c_attack_table[$i] + $c_attack_table1[$i] + $c_attack_table2[$i]);
		}
		if(isset($c_attack_table0[$i]) || isset($c_attack_table1[$i]))
			$c_attack_table_prn[$i] .= "</font>";
	}

	$defence_table_prn[$i] = "".$defence_table[$i];
	if(isset($defence_table0[$i]) || isset($defence_table1[$i]) || isset($defence_table2[$i]))
	{
		$defence_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($defence_table0[$i]) || isset($defence_table1[$i]))
			$defence_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($defence_table0[$i] > $defence_table1[$i])
		{
			$defence_table_prn[$i] .= ($defence_table[$i] + $defence_table0[$i] + $defence_table2[$i]);
		}
		else
		{
			$defence_table_prn[$i] .= ($defence_table[$i] + $defence_table1[$i] + $defence_table2[$i]);
		}
		if(isset($defence_table0[$i]) || isset($defence_table1[$i]))
			$defence_table_prn[$i] .= "</font>";
	}

	$r_defence_table_prn[$i] = "".$r_defence_table[$i];
	if(isset($r_defence_table0[$i]) || isset($r_defence_table1[$i]) || isset($r_defence_table2[$i]))
	{
		$r_defence_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($r_defence_table0[$i]) || isset($r_defence_table1[$i]))
			$r_defence_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($r_defence_table0[$i] > $r_defence_table1[$i])
		{
			$r_defence_table_prn[$i] .= ($r_defence_table[$i] + $r_defence_table0[$i] + $r_defence_table2[$i]);
		}
		else
		{
			$r_defence_table_prn[$i] .= ($r_defence_table[$i] + $r_defence_table1[$i] + $r_defence_table2[$i]);
		}
		if(isset($r_defence_table0[$i]) || isset($r_defence_table1[$i]))
			$r_defence_table_prn[$i] .= "</font>";
	}

	$resist_table_prn[$i] = "".$resist_table[$i];
	if(isset($resist_table0[$i]) || isset($resist_table1[$i]) || isset($resist_table2[$i]))
	{
		$resist_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($resist_table0[$i]) || isset($resist_table1[$i]))
			$resist_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($resist_table0[$i] > $resist_table1[$i])
		{
			$resist_table_prn[$i] .= ($resist_table[$i] + $resist_table0[$i] + $resist_table2[$i]);
		}
		else
		{
			$resist_table_prn[$i] .= ($resist_table[$i] + $resist_table1[$i] + $resist_table2[$i]);
		}
		if(isset($resist_table0[$i]) || isset($resist_table1[$i]))
			$resist_table_prn[$i] .= "</font>";
	}

	$speed_table_prn[$i] = "".$speed_table[$i];
	if(isset($speed_table0[$i]) || isset($speed_table1[$i]) || isset($speed_table2[$i]))
	{
		$speed_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($speed_table0[$i]) || isset($speed_table1[$i]))
			$speed_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($speed_table0[$i] > $speed_table1[$i])
		{
			$speed_table_prn[$i] .= ($speed_table[$i] + $speed_table0[$i] + $speed_table2[$i]);
		}
		else
		{
			$speed_table_prn[$i] .= ($speed_table[$i] + $speed_table1[$i] + $speed_table2[$i]);
		}
		if(isset($speed_table0[$i]) || isset($speed_table1[$i]))
			$speed_table_prn[$i] .= "</font>";
	}

	$r_attack_table_prn[$i] = "".$r_attack_table[$i];
	if(isset($r_attack_table0[$i]) || isset($r_attack_table1[$i]) || isset($r_attack_table2[$i]))
	{
		$r_attack_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($r_attack_table0[$i]) || isset($r_attack_table1[$i]))
			$r_attack_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($r_attack_table0[$i] > $r_attack_table1[$i])
		{
			$r_attack_table_prn[$i] .= ($r_attack_table[$i] + $r_attack_table0[$i] + $r_attack_table2[$i]);
		}
		else
		{
			$r_attack_table_prn[$i] .= ($r_attack_table[$i] + $r_attack_table1[$i] + $r_attack_table2[$i]);
		}
		if(isset($r_attack_table0[$i]) || isset($r_attack_table1[$i]))
			$r_attack_table_prn[$i] .= "</font>";
	}

	$s_range_table_prn[$i] = "".$s_range_table[$i];
	if(isset($s_range_table0[$i]) || isset($s_range_table1[$i]) || isset($s_range_table2[$i]))
	{
		$s_range_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($s_range_table0[$i]) || isset($s_range_table1[$i]))
			$s_range_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($s_range_table0[$i] > $s_range_table1[$i])
		{
			$s_range_table_prn[$i] .= ($s_range_table[$i] + $s_range_table0[$i] + $s_range_table2[$i]);
		}
		else
		{
			$s_range_table_prn[$i] .= ($s_range_table[$i] + $s_range_table1[$i] + $s_range_table2[$i]);
		}
		if(isset($s_range_table0[$i]) || isset($s_range_table1[$i]))
			$s_range_table_prn[$i] .= "</font>";
	}

	$ammo_table_prn[$i] = "".$ammo_table[$i];
	if(isset($ammo_table0[$i]) || isset($ammo_table1[$i]) || isset($ammo_table2[$i]))
	{
		$ammo_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($ammo_table0[$i]) || isset($ammo_table1[$i]))
			$ammo_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($ammo_table0[$i] > $ammo_table1[$i])
		{
			$ammo_table_prn[$i] .= ($ammo_table[$i] + $ammo_table0[$i] + $ammo_table2[$i]);
		}
		else
		{
			$ammo_table_prn[$i] .= ($ammo_table[$i] + $ammo_table1[$i] + $ammo_table2[$i]);
		}
		if(isset($ammo_table0[$i]) || isset($ammo_table1[$i]))
			$ammo_table_prn[$i] .= "</font>";
	}

	$stamina_table_prn[$i] = "".$stamina_table[$i];
	if(isset($stamina_table0[$i]) || isset($stamina_table1[$i]) || isset($stamina_table2[$i]))
	{
		$stamina_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($stamina_table0[$i]) || isset($stamina_table1[$i]))
			$stamina_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($stamina_table0[$i] > $stamina_table1[$i])
		{
			$stamina_table_prn[$i] .= ($stamina_table[$i] + $stamina_table0[$i] + $stamina_table2[$i]);
		}
		else
		{
			$stamina_table_prn[$i] .= ($stamina_table[$i] + $stamina_table1[$i] + $stamina_table2[$i]);
		}
		if(isset($stamina_table0[$i]) || isset($stamina_table1[$i]))
			$stamina_table_prn[$i] .= "</font>";
	}

	$morale_table_prn[$i] = "".$morale_table[$i];
	if(isset($morale_table0[$i]) || isset($morale_table1[$i]) || isset($morale_table2[$i]))
	{
		$morale_table_prn[$i] .= "<br>&#150;<br>";
		if(isset($morale_table0[$i]) || isset($morale_table1[$i]))
			$morale_table_prn[$i] .= "<font color=\"#FFCC00\">";
		if($morale_table0[$i] > $morale_table1[$i])
		{
			$morale_table_prn[$i] .= ($morale_table[$i] + $morale_table0[$i] + $morale_table2[$i]);
		}
		else
		{
			$morale_table_prn[$i] .= ($morale_table[$i] + $morale_table1[$i] + $morale_table2[$i]);
		}
		if(isset($morale_table0[$i]) || isset($morale_table1[$i]))
			$morale_table_prn[$i] .= "</font>";
	}
}
//dumper($r_attack_table_prn,"r_attack_table_prn");

//echo "<table width=100% border=1><tr><td>№</td><td colspan=20>Units</td></tr>";

//dumper($u_abil_c,"COUNT");

//функция формирования умений из массива абилок
function do_abil($i,$abil,$abil_c)//#юнита, массив абилок, массив количества абилок
{
	global $up_type,$unit_abil_spell,$unit_aqua,$up_name,$unit_aqua,$unit_bold_not_magic,$unit_bold;
//$u_abil3,$u_abil_c,$u_abil_c_path_0,$u_abil_c_path_1,
	$p = "";
	for($j=0;$j<=20;$j++)
	{
		foreach($abil[$i][$j] as $v)
		{
			$p .= "<font color=\"green\">";
			...
			if($j != 0)
			{
				...
				$p2 .= (($abil_c[$i][$v]>1) ? " x".$abil_c[$i][$v]."; " : "; ");
			}
			if(in_array($v,$unit_aqua))
			{
				$p .= "<B>Ур.$j:</font> <font color=\"fuchsia\">".$up_name[$v]."</font>";
				$p .= (($abil_c[$i][$v]>1) ? " <font color=\"red\">x".$abil_c[$i][$v]."</B></font>; " : "</B>; ");
			}
			else
			if($aura_fl == 1)
			{
			...
			if(($spell_fl == 1)  && !in_array($v,$unit_bold_not_magic))
			{
				...
				$p .= (($abil_c[$i][$v]>1) ? " <font color=\"red\">x".$abil_c[$i][$v]."</B></I></font>; " : "</B></I>; ");
			}
			else
			...
			else
			{
				...
				$p .= (($abil_c[$i][$v]>1) ? " <font color=\"red\">x".$abil_c[$i][$v]."</font>; " : "; ");
			}
		}
	}
	return $p;
...
//	echo "<tr><td>$i ($u_name[$i])</td><td>";
	...
	if(isset($up_path[$i]))
	{
		$p .= "<B><font color=\"#FFCC00\">".$up_name[$up_path[$i][0]].":</font></B> ";
		$p .= substr(do_abil($i,$u_abil3_path_0,$u_abil_c_path_0),0,-2);
		$p .= "<br><B><font color=\"#FFCC00\">".$up_name[$up_path[$i][1]].":</font></B> ";
		$p .= substr(do_abil($i,$u_abil3_path_1,$u_abil_c_path_1),0,-2);
		$p .= "<br><B><font color=\"#FFCC00\">Общие умения:</font></B> ";
	}
	$p .= do_abil($i,$u_abil3,$u_abil_c);
...
/*	
	...
	echo "</td></tr>";
*/
		echo "<tr><td align=center>$i</td><td>$u_name[$i]</td><td></td><td align=center>".$level_table[$i];
		echo "</td><td align=center>".($up_prn[$i][0][1] != 0 ? $life_table[$i].$up_prn[$i][0][0].($up_prn[$i][0][1] + $life_table[$i]).$up_prn[$i][0][2] : $life_table[$i]);
//		echo "</td><td align=center>".$life_table_prn[$i];
//		if(isset($life_table2[$i])) echo "<br>&#150;<br>".($life_table[$i]+$life_table2[$i]);
		echo "</td><td align=center>".($up_prn[$i][1][1] != 0 ? $attack_table[$i].$up_prn[$i][1][0].($up_prn[$i][1][1] + $attack_table[$i]).$up_prn[$i][1][2] : $attack_table[$i]);
//		echo "</td><td align=center>".$attack_table_prn[$i];
//		if(isset($attack_table2[$i])) echo "<br>&#150;<br>".($attack_table[$i]+$attack_table2[$i]);
		echo "</td><td align=center>".($up_prn[$i][2][1] != 0 ? $c_attack_table[$i].$up_prn[$i][2][0].($up_prn[$i][2][1] + $c_attack_table[$i]).$up_prn[$i][2][2] : $c_attack_table[$i]);
//		echo "</td><td align=center>".$c_attack_table_prn[$i];
//		if(isset($c_attack_table2[$i])) echo "<br>&#150;<br>".($c_attack_table[$i]+$c_attack_table2[$i]);
		echo "</td><td align=center>".($up_prn[$i][3][1] != 0 ? $defence_table[$i].$up_prn[$i][3][0].($up_prn[$i][3][1] + $defence_table[$i]).$up_prn[$i][3][2] : $defence_table[$i]);
//		echo "</td><td align=center>".$defence_table_prn[$i];
//		if(isset($defence_table2[$i])) echo "<br>&#150;<br>".($defence_table[$i]+$defence_table2[$i]);
		echo "</td><td align=center>".($up_prn[$i][4][1] != 0 ? $r_defence_table[$i].$up_prn[$i][4][0].($up_prn[$i][4][1] + $r_defence_table[$i]).$up_prn[$i][4][2] : $r_defence_table[$i]);
//		echo "</td><td align=center>".$r_defence_table_prn[$i];
//		if(isset($r_defence_table2[$i])) echo "<br>&#150;<br>".($r_defence_table[$i]+$r_defence_table2[$i]);
		echo "</td><td align=center>".($up_prn[$i][5][1] != 0 ? $resist_table[$i].$up_prn[$i][5][0].($up_prn[$i][5][1] + $resist_table[$i]).$up_prn[$i][5][2] : $resist_table[$i]);
//		echo "</td><td align=center>".$resist_table_prn[$i];
//		if(isset($resist_table2[$i])) echo "<br>&#150;<br>".($resist_table[$i]+$resist_table2[$i]);
		echo "</td><td align=center>".($up_prn[$i][6][1] != 0 ? $speed_table[$i].$up_prn[$i][6][0].($up_prn[$i][6][1] + $speed_table[$i]).$up_prn[$i][6][2] : $speed_table[$i]);
//		echo "</td><td align=center>".$speed_table_prn[$i];
//		if(isset($speed_table2[$i])) echo "<br>&#150;<br>".($speed_table[$i]+$speed_table2[$i]);
		echo "</td><td align=center>".($up_prn[$i][7][1] != 0 ? $r_attack_table[$i].$up_prn[$i][7][0].($up_prn[$i][7][1] + $r_attack_table[$i]).$up_prn[$i][7][2] : $r_attack_table[$i]);
//		echo "</td><td align=center>".$r_attack_table_prn[$i];
//		if(isset($r_attack_table2[$i])) echo "<br>&#150;<br>".($r_attack_table[$i]+$r_attack_table2[$i]);
		echo "</td><td align=center>".($up_prn[$i][8][1] != 0 ? $s_range_table[$i].$up_prn[$i][8][0].($up_prn[$i][8][1] + $s_range_table[$i]).$up_prn[$i][8][2] : $s_range_table[$i]);
//		echo "</td><td align=center>".$s_range_table_prn[$i];
//		if(isset($s_range_table2[$i])) echo "<br>&#150;<br>".($s_range_table[$i]+$s_range_table2[$i]);
		echo "</td><td align=center>".($up_prn[$i][9][1] != 0 ? $ammo_table[$i].$up_prn[$i][9][0].($up_prn[$i][9][1] + $ammo_table[$i]).$up_prn[$i][9][2] : $ammo_table[$i]);
//		echo "</td><td align=center>".$ammo_table_prn[$i];
//		if(isset($ammo_table2[$i])) echo "<br>&#150;<br>".($ammo_table[$i]+$ammo_table2[$i]);
		echo "</td><td align=center>".($up_prn[$i][10][1] != 0 ? $stamina_table[$i].$up_prn[$i][10][0].($up_prn[$i][10][1] + $stamina_table[$i]).$up_prn[$i][10][2] : $stamina_table[$i]);
//		echo "</td><td align=center>".$stamina_table_prn[$i];
//		if(isset($stamina_table2[$i])) echo "<br>&#150;<br>".($stamina_table[$i]+$stamina_table2[$i]);
		echo "</td><td align=center>".($up_prn[$i][11][1] != 0 ? $morale_table[$i].$up_prn[$i][11][0].($up_prn[$i][11][1] + $morale_table[$i]).$up_prn[$i][11][2] : $morale_table[$i]);
//		echo "</td><td align=center>".$morale_table_prn[$i];
...
*/
/*		echo "<tr><td align=center>$i</td><td>$u_name[$i]</td><td></td><td align=center>";
		echo $level_table[$i]."</td><td align=center>".$life_table_prn[$i];
//		if(isset($life_table2[$i])) echo "<br>&#150;<br>".($life_table[$i]+$life_table2[$i]);
		echo "</td><td align=center>".$attack_table_prn[$i];
//		if(isset($attack_table2[$i])) echo "<br>&#150;<br>".($attack_table[$i]+$attack_table2[$i]);
		echo "</td><td align=center>".$c_attack_table_prn[$i];
