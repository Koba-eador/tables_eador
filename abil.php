...
$abil_numeric[205]=1;//Умелый взломщик
...
//$abil_num[220]=999;//Заклинание по всем рядом стоящим существам при смерти (невидимая абилка)
$abil_name[220] = "Заклинание по всем рядом стоящим существам при смерти";
$abil_name[224]="Удержание строя";
$abil_txt2[224]="Приоритет для удержания позиции.";
$abil_name[229]="Игнорирование брони при ударе в спину";
$abil_txt2[229]="Игнорирование брони при ударе в спину.";
$abil_name[254]="Смерть существа не влияет на боевой дух окружающих";
$abil_txt2[254]="Смерть существа не влияет на боевой дух окружающих.";
$abil_name[255]="Ловушка: наложение заклятья";
$abil_txt2[255]="Ловушка: наложение заклятья.";
$abil_name[256]="Ловушка: отъятие очков действия";
$abil_txt2[256]="Ловушка: увеличение затрат скорости на прохождение через тайл на %d.";
//$abil_numeric[256]=1;
$abil_name[257]="Ловушка: урон наступившему";
$abil_txt2[257]="Ловушка: урон наступившему: %d.";
//$abil_numeric[257]=1;
$abil_name[258]="Ловушка: наложение наступившему способности \"Не сражается\"";
$abil_txt2[258]="Ловушка: наложение наступившему способности \"Не сражается\".";
$abil_name[268] = "Переваривание";
$abil_num[268] = 528;//заглушка: Переваривание = Проглатывание
$abil_txt2[268] = "Переваривание %d";
$abil_numeric[268]=1;
//$abil_txt2[284]="Добавочная плата наёмника снижена на 25%";
$abil_name[294]="Держать строй!";
$abil_txt2[294]="Предпочтение воином оборонительной тактики.";
...
$abil_txt2[357]="Начальная форма Меняющего Облик.";
...
//						$up_abil[$n] .= "Аура: <B><font color=\"fuchsia\">".$abil_name[$num3]."</font></B> ".($qua<0 ? "<font color=\"red\">$qua" : "<font color=\"green\">+$qua").($abil_percent[$num]==1 ? "%" : "")."</font></B>; ";
						$up_abil[$n] .= "<B><font color=\"aqua\">Аура</font>: <font color=\"fuchsia\">";
...
*/				else
				...
				if($num==207)//Воин требует меньшего содержания золота (значение умения - это коэффициент (в процентах), не может быть больше 100)
					$up_abil[$n] .= "Содержание воина <B>".($qua>0 ? "<font color=\"green\">".(-$qua) : "<font color=\"red\">+".abs($qua))."%</font></B> (золото); ";
				else
				if($num==208)//Воин требует меньшего содержания кристаллов (значение умения - это коэффициент (в процентах), не может быть больше 100)
					$up_abil[$n] .= "Содержание воина <B>".($qua>0 ? "<font color=\"green\">".(-$qua) : "<font color=\"red\">+".abs($qua))."%</font></B> (кристаллы); ";
				else
				if($num==220)//Заклинание по всем рядом стоящим существам при смерти (невидимая абилка).
				{
					$up_abil[$n] .= "Применение заклятья <B><font color=\"blue\">".$spell_name[$qua]."</font></B> [<font color=\"fuchsia\">".trim($spell_txt[$qua])."</font>] по всем рядом стоящим существам при смерти цели; ";
				}
				else
...
*/
				...
				if($num==374)//Снижение платы
				{
					$up_abil[$n] .= "\"Снижение платы на <B>$qua%</B>\"; ";
				}
/*					else
...
$PIC = 1;//Переключатель - вывод иконок (=1)/без картинок (для копипасты)
...
//	echo "<span style='color:fuchsia'><a name=\"e".$i."\"></a><B>/".$i." <img src=i/u".$i.".gif align=center> ".$up_name[$i]."</B></span><br>";
	$num = count($up_type_prn[$i]);
	for($j=0;$j<$num;$j++)
	{
		$type=$up_type_prn[$i][$j];
		$qua=$up_quantity_prn[$i][$j];
//		$aura_flag = 0;
		if($type>3000)//Аура
		{
			$type -= 3000;
		...
		if($type==162)//Смена формы
		{
			$abil_txt_prn[$abil_num[$type]] = "Способность в бою превращаться в <B><font color=\"brown\">".$u_name[$qua]."</font></B>";
		}
		else
...
*/
		...
		if($type==284)//Добавочная плата наёмника снижена на 25%.
		{
			$abil_txt_prn[$abil_num[$type]] = "Добавочная плата наёмника снижена на <B><font color=\"green\">25</font></B>%.";
		}
		else
		if($type==220)//Заклинание по всем рядом стоящим существам при смерти (невидимая абилка)
		{
//			$s=explode("%s",$abil_txt[$abil_num[$type]]);
			$abil_txt_prn[$abil_num[$type]] = "Применение заклятья <B><font color=\"blue\">".$spell_name[$qua]."</font></B> по всем рядом стоящим существам при смерти цели.";
		}
		else
...
//			$abil_txt[83] = str_replace("%b","<B><font color=\"green\">".($qua*2)."</font></B>",$abil_txt[83]);
		...
		if($j==0)
		{
			...
			echo "<td align=center rowspan=$num class=bottom>";
			if($PIC==1)
				echo "<img align=center src=i/u".$i.".gif>";
			echo "</td>";
...
//			echo "<td></td><td></td>";
		if($j==$num-1)
		{
			...
			if($PIC==1)
				echo "<img align=center src=i/a".($abil_num[$type]+1-1).".gif>";
...
//			echo "</td><td class=bottom>";
		...
		{
			echo "<td>".$abil_name[$type]."</td><td align=center>";
			if($PIC==1)
				echo "<img align=center src=i/a".($abil_num[$type]+1-1).".gif>";
			echo "</td><td>";
...
{
	echo "<tr><td align=center>$i</td><td>$effects_name[$i]</td><td align=center>";
	if($PIC==1)
	{
		echo "<img align=center src=i/";
//	echo isset($effects_ability[$i]) ? "a".$abil_num[$effects_ability[$i]] : "s".$effects_spell[$i];
	...
	echo "</td><td>".$effects_txt[$i]."</td><td>";
	echo "ABIL=".($effects_ability[$i] ? $abil_num[$effects_ability[$i]]." (".$abil_name[$effects_ability[$i]].")" : "")."<br>";
